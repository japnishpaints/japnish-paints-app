...
let state={role:"",token:localStorage.getItem("jp_token")||"",user:null,permissions:null,screen:"home",scanner:null,cameraStream:null,cameraTimer:null,branding:null};

function siteUrl(path){
  const v=String(path||'').trim();
  if(!v)return '';
  if(/^https?:\/\//i.test(v) || v.startsWith('data:') || v.startsWith('blob:')) return v;
  const base=(()=>{try{return new URL(C.API_BASE_URL||'https://japnishpaints.store/api').origin}catch(e){return 'https://japnishpaints.store'}})();
...
function brandImageUrl(value){
  ...
  return siteUrl('/uploads/'+v.replace(/^\/+/, ''));
}
...
async function loadLoginSliders(){
  try{const d=await dataApi('?action=public_sliders');loginSlides=(d.slides||[]).map(x=>({...x,image_url:x.id?siteUrl('/mobile/slider-image.php?id='+encodeURIComponent(x.id)):brandImageUrl(x.image_url)}));}catch(e){loginSlides=[]}
...
function logout(){stopScan();localStorage.removeItem("jp_token");state={role:"",token:"",user:null,permissions:null,screen:"home",scanner:null,cameraStream:null,cameraTimer:null};loginView()}
(async function boot(){
